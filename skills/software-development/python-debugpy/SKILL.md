...
10. **Stale `.pyc` bytecode with long-running processes.** When modifying `.py` source files of a daemon or gateway (like `gateway/run.py`) that runs continuously, Python may serve the old `.pyc` from `__pycache__/` instead of recompiling from the new source. This manifests as: source file contains the fix, but the running process exhibits old behavior.

    **Symptom:** `read_file` shows the fix in the source, but the code doesn't behave as expected. The loaded module's `__file__` points to the right file, yet `inspect.getsource()` returns old code.

    **Diagnosis:** Check if the `.pyc` is older than the source:
    ```bash
    stat --format='%Y %n' /path/to/module.py /path/to/__pycache__/module.cpython-*.pyc
    ```
    If the `.pyc` timestamp is newer or the bytecode differs, it's stale.

    **Fix:** Kill the process and delete `__pycache__/` before restart:
    ```bash
    rm /path/to/__pycache__/module.cpython-*.pyc
    kill -9 <pid>
    ```
    On restart, Python recompiles `.pyc` from the fresh source.
...
- **`references/footer-injection-debug.md`** — Common issues debugging Hermes gateway footer injection (silent TypeErrors, stale `.pyc`, wrong data sources, `HERMES_QUIET` log suppression)
