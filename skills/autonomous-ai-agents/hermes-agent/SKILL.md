...
### Footer injection shows `⏱0.0s · 🔁0` (all zeros)
Footer values are 0 because the call to `build_footer_line()` passes `text=` as keyword arg but the function parameter is named `response_text` — this raises `TypeError` which gets caught by `except Exception: pass` silently. Fix: use `response_text=` keyword. Also ensure `model_name` comes from `agent_result.get("model")` not `hook_ctx` (hook_ctx lacks `model_name`). `context_usage` must be computed from `input_tokens` + `output_tokens` divided by context window.
...
When `HERMES_QUIET=1` is set (common in production gateway), the logging level is WARN+. This means `logger.info()` and `logger.debug()` output is suppressed. For debugging, use `logger.warning()` or higher level. Gateway logs go to `~/.hermes/logs/gateway.log`.
