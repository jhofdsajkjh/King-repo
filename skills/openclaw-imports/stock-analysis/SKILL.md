...
> **用户格式偏好（必须遵守）**：分析结果需推送飞书时，一律使用**手机排版**格式：
> - 每只股票用分隔线 `---` 分开，不要用表格（手机屏幕窄，表格挤压难读）
...
- `references/eastmoney-a-share-api.md` — A股实时数据提取 API 字段速查表
- `references/feishu-stock-card-format.md` — 飞书股票卡片标准格式（实测验证，2026-05-21）
